...
Two distinct kinds of pages exist in this project, and they must not be confused during build:

1. **Static/landing pages** — hand-built directly as `.astro` files (homepage, campaign/landing pages, anything with bespoke layout or interactive widgets like the AviationStack search bars). These do **not** pull content from WordPress, and prerender to plain HTML at build time.
2. **Content pages** — Pages published in WordPress. Astro fetches these live, per request (title, content HTML, parent relationship, assigned template), through a short in-memory cache, and renders them through a shared layout system. Editors control these entirely from WP admin; an edit shows up on the next request once the cache entry expires — no rebuild, no redeploy, nothing to trigger from WP's side (see rendering-mode decision in `IMPLEMENTATION.md`).
